...
export default function Edit({ role, employees }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        assigned_employees: role.employees?.map(emp => emp.id) || [],
    });
    ...
    const handleSelectAll = (group) => {
        ...
        const hasAllPermissions = groupPermissions.every(p => data.permissions.includes(p));
        
        if (hasAllPermissions) {
            // Remove all permissions from this group
            const newPermissions = data.permissions.filter(p => !groupPermissions.includes(p));
            setData('permissions', newPermissions);
        } else {
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setData('access_level', parseInt(e.target.value))}
                                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-transparent dark:bg-gray-700 dark:text-white"
                                    >
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <label className="flex items-center">
                                        ...
                                        <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">
                                            This role is active and can be assigned to employees
                                        </span>
                                    </label>
                                </div>
                            </div>
                                        ...
                                        {colorOptions.map((color) => (
                                            <button
                                                ...
                                                className={`w-8 h-8 rounded-full ${color.preview} border-2 ${
                                                    data.color === color.value ? 'border-gray-900' : 'border-transparent'
                                        ...
                                        {iconOptions.map((icon) => (
                                            <button
                                                ...
                                                className={`w-8 h-8 text-lg hover:scale-110 transition-transform ${
                                                    data.icon === icon ? 'ring-2 ring-purple-500' : ''
                                                }`}
                                            >
                                                {icon}
                                            </button>
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="flex gap-2 mb-2">
                                            <button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setData('assigned_employees', allEmployeeIds);
                                                }}
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setData('assigned_employees', [])}
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setData('assigned_employees', selectedOptions);
                                            }}
                                    ...
                                    {data.assigned_employees.length > 0 && (
                                        <div className="mt-3 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                                            <p className="text-sm text-purple-700 dark:text-purple-300">
                                                <strong>Selected:</strong> {data.assigned_employees.length} employee(s)
                                            </p>
                                        </div>
                                    )}
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                            ...
                            <div className="space-y-4">
                                {permissionOptions.map((group) => (
                                    <div key={group.group} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4">
                                        <div className="flex items-center justify-between mb-3">
                                            ...
                                            >
                                                {group.permissions.every(p => data.permissions.includes(p)) ? 'Deselect All' : 'Select All'}
                                            </button>
                                        </div>
